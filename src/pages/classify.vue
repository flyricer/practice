...
<transition name="slide">
   	<div class="classify">
	   	...
	   	<div id="wrapper" ref="scrollWrap">
	       	<div class="scroller">
	      		<ul>
		  			<li class="film-list" v-for="v in goodsList" :key="v.id" tag="li" @click="todetail(v.id)">
			    		<div class="film-list__img">
		   			   		 <img v-lazy="v.images.large" alt="" />                
			    		</div>
...
export default {
    ...
    mounted(){ 
      ...
      filterDirectors(arr){
	    ...
	    requestData(){	    
	    	this.showLoading=true;
	    	this.axios.get(`/api/${this.type}?count=10&start=0`)
	   	    	.then((res)=>{ 	  
        ...
        this.scroller = new BScroll(this.$refs.scrollWrap,{
				...
				/*上拉加载更多*/
				this.scroller.on('pullingUp',()=>{this.pullUpEvent()});
			},
			
			pullUpEvent(){   
	    	if(!this.noMoreData){
		    	this.beforeFetch("Up");
			    	this.axios.get(`/api/${this.type}?count=10&start=${(++this.currentPage)*10}`)
				    	.then((res)=>{ 	  
			   	    		res=res.data;
	    ...
	    pullDownEvent(){ 
	    	...
	    	setTimeout(()=>{
		    	this.axios.get(`/api/${this.type}?count=10&start=${Math.floor(Math.random()*100)}`)
			    	.then((res)=>{
				    	res=res.data;
				    	if(res.subjects.length>0){
